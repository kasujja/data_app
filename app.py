...
from handler import Students , StudentTransactions,Transactions,Daily, Weekly,Savings,SavingTransactions,Parents,Schools,SavingBalance,ParentTransactions,ParentAccounts
...
parent_accounts_handler = ParentAccounts()
...
parent_accounts_data = parent_accounts_handler.getParentAccounts()
...
with tab3:
    ...
    with trns:
        ...
        with lin:
            cumx=px.bar(transactions_data, x="month", y="amount",title='Total Monthly Transactions')
            cumx.update_layout(
                height=500
            )
